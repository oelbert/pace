...
class LocalCommConfig(CreatesComm):
    """
    Configuration for a LocalComm object which performs halo hupdates locally using a
    buffer dictionary.

    Attributes:
        rank: rank of the comm
        total_ranks: the total number of ranks for the comm to pretend to have
        buffer_dict: dictionry to buffer halo exchange messages
    """
    ...
    def get_comm(self):
        return LocalComm(
            rank=self.rank, total_ranks=self.total_ranks, buffer_dict=self.buffer_dict
        )
